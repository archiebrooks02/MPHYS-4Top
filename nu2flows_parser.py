...
def main(infile_name,outfile_name,split_ratio):
    ...
    reco  = uproot.open(directory + f"{infile_name}:Reco")
    truth = uproot.open(directory + f"{infile_name}:Truth")
    ...
    split_ratio_int = int(split_ratio)
    test_mask = np.arange(0,len(jet_data))%split_ratio_int==0
