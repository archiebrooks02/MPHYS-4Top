...
def loader(train_file,test_file):
...
def objective(trial):
    ...
    hidden_size_2 = trial.suggest_int('hidden_size_2', 50, 100)
    dropout_rate = trial.suggest_float('dropout_rate', 0.2, 0.5)
    learning_rate = trial.suggest_float('learning_rate', 1e-5, 1e-3)
    
    NN_clf = Classifier_MLP(in_dim=input_dim, hidden_dim1 = hidden_size_1, hidden_dim2 = hidden_size_2, dropout_rate=dropout_rate, out_dim=output_dim)
    optimiser = torch.optim.Adam(NN_clf.parameters(),lr=learning_rate)
    NN_clf.fit(train_loader, valid_loader, optimiser, verbose=False)
...
def run_optuna():
    ...
    study.optimize(objective, n_trials=100)  # Run for 100 trials or 1 hour

    print("Best parameters:", study.best_params)
    print("Best value (AUC):", study.best_value)

    # Plot hyperparameter importance (AUC vs hyperparameter)
    fig = vis.plot_optimization_history(study)
    fig.show()

    # Visualize the relationship between hyperparameters and the AUC
    fig = vis.plot_parallel_coordinate(study)
    fig.show()

    # Visualize the hyperparameter AUC correlations
...
if __name__ == "__main__":
    ...
    train_loader, valid_loader,X_test_variable, y_test_variable = loader(args.train_file, args.test_file)
    run_optuna()
